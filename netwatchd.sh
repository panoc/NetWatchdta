...
echo "🚀 Starting netwatchd installation..."

# 1. Dependency Check (Discord needs curl)
...
while true; do
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [ "$IS_INTERNET_DOWN" -eq 0 ]; then
            if [ -f "$F_Q_REC" ]; then
                DUR_REC=$(cat "$F_Q_REC")
                curl -s -H "Content-Type: application/json" -X POST -d "{\"content\": \"🚨 **$NAME** went DOWN (Queued)\"}" "$DISCORD_URL" > /dev/null 2>&1
                sleep 1
                curl -s -H "Content-Type: application/json" -X POST -d "{\"content\": \"✅ **RECOVERY**: **$NAME** is ONLINE\n⏱️ **Down for:** $DUR_REC\"}" "$DISCORD_URL" > /dev/null 2>&1
                rm -f "$F_DOWN" "$F_Q_REC" "$F_Q_FAIL"
            elif [ -f "$F_Q_FAIL" ]; then
                curl -s -H "Content-Type: application/json" -X POST -d "{\"content\": \"🚨 **$NAME** is DOWN! (Delayed)\"}" "$DISCORD_URL" > /dev/null 2>&1
                rm "$F_Q_FAIL"
            fi
        fi
    done < "$IP_LIST_FILE"
...
echo "⚙️  Settings: $INSTALL_DIR/netwatchd_settings.conf"
echo "📝 IP List:  $INSTALL_DIR/netwatchd_ips.conf"