...
### Network Monitoring & Alerting for OpenWrt & Linux

**netwatchdta** is a lightweight, POSIX-compliant network monitor designed specifically for **OpenWrt routers** and **Linux servers**. It bridges the gap between your network hardware and your mobile device by sending real-time status **notifications** via **Discord** and/or **Telegram**.
...
* **🧠 Smart RAM Detection**
* **Parallel Mode:** Automatically enables on systems with **≥ 256MB RAM** for ultra-fast, simultaneous pinging.
* **Sequential Mode:** Automatically enables on systems with **< 256MB RAM** to prevent instability and OOM crashes.
...
> **Tip:** The installer will ask for these during setup. You can update them securely later using the `credentials` command.
...
### Option B: Universal Linux
Compatible with most Linux distros :  ✅ Ubuntu, Debian, Mint, Kali, Raspberry Pi OS ✅ Fedora, CentOS, RHEL, AlmaLinux, Rocky ✅ Manjaro, Arch Linux, EndeavourOS

**Requirement:** `sudo` privileges. The script auto-detects your package manager to install dependencies.

```sh
sudo wget --no-check-certificate -qO /tmp/install_netwatchdta.sh "https://raw.githubusercontent.com/panoc/NetWatchdta/refs/heads/main/Linux/install_netwatchdta_linux.sh" && sudo bash /tmp/install_netwatchdta.sh

```

### The interactive installer will:

1. Check system requirements (Storage/RAM).
...
Configuration files are located in the Base Folder (see above).

<details>

<summary><strong>1. nwdta_settings.conf - Controls the behavior of the daemon.</strong></summary>
...
| `EXT_IP2` | Secondary external IP to ping (Default: `8.8.8.8`). |
...
<summary><strong>2. nwdta_ips.conf - List the local devices you want to monitor.</strong></summary>
...
| OpenWrt Command | Linux Command | Action |
| --- | --- | --- |
...
If you prefer the web interface on OpenWrt, you can add control buttons to your router's dashboard.
...
To completely remove `netwatchdta`, use the built-in purge command shown in the table above, or use the one-line uninstaller:
...
```sh
wget --no-check-certificate -qO /tmp/uninstall_netwatchdta.sh "https://raw.githubusercontent.com/panoc/NetWatchdta/refs/heads/main/Linux/uninstall_netwatchdta_linux.sh" && sudo bash /tmp/uninstall_netwatchdta.sh

```

You will be prompted to either remove everything (including dependencies) or keep your configuration files for a future reinstall.
